...
#Region "Public Functions"
    ...
    Public Function GetInstrumentToken(ByVal tableName As DataBaseTable, ByVal tradingDate As Date, ByVal tradingSymbol As String) As String
        ...
        Select Case tableName
            Case DataBaseTable.Intraday_Cash, DataBaseTable.EOD_Cash, DataBaseTable.EOD_POSITIONAL
                cm = New MySqlCommand("SELECT DISTINCT(`INSTRUMENT_TOKEN`) FROM `active_instruments_cash` WHERE `TRADING_SYMBOL` = @trd AND `AS_ON_DATE`<=@sd", conn)
                cm.Parameters.AddWithValue("@trd", String.Format("{0}", tradingSymbol))
            Case DataBaseTable.Intraday_Currency, DataBaseTable.EOD_Currency
                cm = New MySqlCommand("SELECT DISTINCT(`INSTRUMENT_TOKEN`) FROM `active_instruments_currency` WHERE `TRADING_SYMBOL` = @trd AND `AS_ON_DATE`<=@sd", conn)
                cm.Parameters.AddWithValue("@trd", String.Format("{0}", tradingSymbol))
            Case DataBaseTable.Intraday_Commodity, DataBaseTable.EOD_Commodity
                cm = New MySqlCommand("SELECT DISTINCT(`INSTRUMENT_TOKEN`) FROM `active_instruments_commodity` WHERE `TRADING_SYMBOL` = @trd AND `AS_ON_DATE`<=@sd", conn)
                cm.Parameters.AddWithValue("@trd", String.Format("{0}", tradingSymbol))
            Case DataBaseTable.Intraday_Futures, DataBaseTable.EOD_Futures
                cm = New MySqlCommand("SELECT DISTINCT(`INSTRUMENT_TOKEN`) FROM `active_instruments_futures` WHERE `TRADING_SYMBOL` = @trd AND `AS_ON_DATE`<=@sd", conn)
                cm.Parameters.AddWithValue("@trd", String.Format("{0}", tradingSymbol))
        End Select

        OnHeartbeat(String.Format("Getting current trading symbol and token from DataBase for {0} on {1}", tradingSymbol, tradingDate.ToShortDateString))
    ...
    Public Async Function GetHistoricalDataForSpecificTradingSymbolAsync(ByVal tableName As DataBaseTable, ByVal tradingSymbol As String, ByVal startDate As Date, ByVal endDate As Date) As Task(Of Dictionary(Of Date, Payload))
        Dim ret As Dictionary(Of Date, Payload) = Nothing
        Dim instrumentToken As String = Nothing
        Dim ZerodhaEODHistoricalURL As String = "https://kitecharts-aws.zerodha.com/api/chart/{0}/day?api_key=kitefront&access_token=K&from={1}&to={2}"
        Dim ZerodhaIntradayHistoricalURL As String = "https://kitecharts-aws.zerodha.com/api/chart/{0}/minute?api_key=kitefront&access_token=K&from={1}&to={2}"
        ...
        Select Case tableName
            ...
            Case DataBaseTable.Intraday_Cash, DataBaseTable.Intraday_Commodity, DataBaseTable.Intraday_Currency, DataBaseTable.Intraday_Futures
                ZerodhaHistoricalURL = ZerodhaIntradayHistoricalURL
        End Select
        instrumentToken = GetInstrumentToken(tableName, endDate.Date, tradingSymbol)
        If instrumentToken IsNot Nothing AndAlso instrumentToken <> "" Then
            Dim historicalDataURL As String = String.Format(ZerodhaHistoricalURL, instrumentToken, startDate.ToString("yyyy-MM-dd"), endDate.ToString("yyyy-MM-dd"))
            OnHeartbeat(String.Format("Fetching historical Data: {0}", historicalDataURL))
            Dim historicalCandlesJSONDict As Dictionary(Of String, Object) = Nothing
    ...
    Public Function GetAllStockList(ByVal tableName As DataBaseTable, ByVal tradingDate As Date) As List(Of String)
        ...
        Select Case tableName
            ...
            Case DataBaseTable.Intraday_Futures, DataBaseTable.EOD_Futures
                cm = New MySqlCommand("SELECT `TRADING_SYMBOL` FROM `active_instruments_futures` WHERE `SEGMENT`='NFO-FUT' AND `AS_ON_DATE`=@sd", conn)
        End Select
